...
void irr::newton::Hidden::NewtonDebugCollision(const NewtonBody* body, int vertexCount, const float* faceVertec, int id)
{
	int i;
	core::vector3df line;
		...
		const NewtonContact* contact)
{
	...
	irr::newton::IMaterialPairAndContact material_pair_and_contact(
		...
		const_cast<NewtonContact*>(contact)
		);
...
void irr::newton::Hidden::defBodyTransformCallback(const NewtonBody* body, const float* matrix) {
...
void irr::newton::Hidden::defBodyForceAndTorqueCallback (const NewtonBody* n_body) 
{

	irr::newton::IBody* body = (irr::newton::IBody*)NewtonBodyGetUserData(n_body);
	irr::newton::Hidden::defBodyForceAndTorqueCallback(body, n_body);
...
//camera callbacks
void irr::newton::Hidden::defCameraBodyTransformCallback(const NewtonBody* body, const float* matrix) {
	irr::newton::Hidden::defBodyTransformCallback(body,matrix);
	irr::newton::Hidden::correctCameraTarget(
		(scene::ICameraSceneNode*)((irr::newton::IBody*)NewtonBodyGetUserData(body))->getNode());
}

void irr::newton::Hidden::defCameraCharacterControllerTransformCallback(const NewtonBody* body, const float* matrix) {

	irr::newton::Hidden::defCharacterControllerTransformCallback(body,matrix);
...
void irr::newton::Hidden::defCharacterControllerTransformCallback(const NewtonBody* body, const float* matrix) {
...
void irr::newton::Hidden::VehicleTireCallBack(const NewtonJoint* vehicle_joint) {
	...
	for(irr::u32 i=0;i<tires.size();i++) {
		...
		irr::f32 omega = NewtonVehicleGetTireOmega (vehicle_joint, tires[i]->tire_id);
		...
		//SIDE SLEEP
		float c_speed=NewtonVehicleGetTireLongitudinalSpeed(vehicle_joint,tires[i]->tire_id);
		...
		c_speed = NewtonVehicleGetTireLongitudinalSpeed (vehicle_joint, tires[i]->tire_id);
...
//RAGDOLL CALLBACKS
void irr::newton::Hidden::defRagDollForceCallback(const NewtonBody* n_body) {

	void* data = NewtonBodyGetUserData(n_body);
	
	//the user data of the body is the NewtonRagDollBone* pointer (as described in newton doc)
	NewtonRagDollBone* n_bone = (NewtonRagDollBone*)NewtonBodyGetUserData(n_body);

	//the user data of the newton rag doll bone pointer is the irr::newton::IRagDollBone*
	irr::newton::IRagDollBone* bone = (irr::newton::IRagDollBone*)NewtonRagDollBoneGetUserData(n_bone);

	//get the body from the irrnewt rag doll bone
	irr::newton::IBody* body = bone->getBody();

	//apply forces and torques

	//-------------------------------------------------------------------------------

	//call default body force callback
	newton::Hidden::defBodyForceAndTorqueCallback( body, n_body );
...
void irr::newton::Hidden::defRagDollTransformCallback(const NewtonRagDollBone* bone)
{
	...
	model = ((irr::newton::IRagDollBone*) NewtonRagDollBoneGetUserData (bone))->getNode();

	// get the local matrix for this bone
	NewtonRagDollBoneGetLocalMatrix (bone, irr::newton::Hidden::GetMatrixPointer(matrix) );
	...
	NewtonGetEulerAngle ( newton::Hidden::GetMatrixPointer(matrix) , &euler.X);

	//ORIGINAL (nor working)
	//model->setRotation(euler * InvPi);
	*/

	//set the rotatino of the node
	model->setRotation(matrix.getRotationDegrees());
