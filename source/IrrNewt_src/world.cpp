...
irr::newton::IWorld::IWorld(irr::IrrlichtDevice* Device) {
	...
	//call newton update 60 times per seconds for default
	this->update_fps = 60.0f;
...
void irr::newton::IWorld::update(irr::f32 time_step) {
	...
	this->time_elapsed=this->device->getTimer()->getRealTime()-last_time;
...
void irr::newton::IWorld::update() {
   ...
   {
      NewtonUpdate(this->world, (newton_time_step / 1000.0f ));   // convert to seconds
...
irr::newton::IBody*  irr::newton::IWorld::createBody(SBodyFromNode bodyData) {
	...
	//autodetect collision type

	if(body.Type == EBT_AUTODETECT)
		return this->createBodyAuto(body.Node,
		body.Mesh,body.Mass,body.TerrainLOD,
		body.BodyOffsetFromNode, body.BodyScaleOffsetFromNode,
		body.calculateOffsetMode, body.calculateSizeMode);

	//primitives, convex hulls and null collisions
	else if(
		body.Type == EBT_PRIMITIVE_BOX              ||
		body.Type == EBT_PRIMITIVE_ELLIPSOID        ||
		body.Type == EBT_PRIMITIVE_CYLINDER         ||
		body.Type == EBT_PRIMITIVE_CHAMFER_CYLINDER ||
		body.Type == EBT_PRIMITIVE_CONE             ||
		body.Type == EBT_PRIMITIVE_CAPSULE          ||
		body.Type == EBT_CONVEX_HULL                ||
		body.Type == EBT_CONVEX_HULL_MODIFIER       ||
		body.Type == EBT_NULL
		) {

		return this->createBody(body.Node,body.Mesh, body.Type ,
		body.Mass,body.BodyOffsetFromNode,body.BodyScaleOffsetFromNode,
		body.calculateOffsetMode, body.calculateSizeMode);
	...
	if(body.Type == EBT_TREE)
		return (irr::newton::IBody*)this->createTreeBody(body.Node,body.Mesh);

	else if(body.Type == EBT_TREE_TERRAIN)
		return (irr::newton::IBody*)this->createTerrainTreeBody((scene::ITerrainSceneNode*)body.Node,body.TerrainLOD);
...
irr::newton::IRagDoll* irr::newton::IWorld::createRagDoll() {

	irr::newton::IRagDoll* v=new irr::newton::IRagDoll(
		this
		);
...
irr::newton::IRagDoll* irr::newton::IWorld::createRagDoll(irr::newton::SRagDoll data) {

	irr::newton::IRagDoll* ragdoll = new newton::IRagDoll(this);

	ragdoll->build(data.mainNode, data.bonesMass);

	irr::newton::Hidden::AddObjectToDestroy(ragdoll);
...
irr::f32 irr::newton::IWorld::getTimeStep() {
	return NewtonGetTimeStep(this->world);
}
