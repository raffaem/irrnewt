...
void irr::newton::IBody::setAutoFreeze(bool value) {
	NewtonBodySetAutoFreeze(this->body, value);
}
...
void irr::newton::IBody::setFreeze(bool value) {
	if(value)
		NewtonWorldFreezeBody(this->getNewtonWorld(),this->body);
	else if(!value)
		NewtonWorldUnfreezeBody(this->getNewtonWorld(),this->body);
}
...
void irr::newton::IBody::drawDebugInfo() {
	...
	NewtonBodyForEachPolygonDo(this->body, irr::newton::Hidden::NewtonDebugCollision);
...
{
	all_poligons.clear();
	NewtonBodyForEachPolygonDo(this->body,NewtonGetAllPoligonsCallback);
	return all_poligons;
