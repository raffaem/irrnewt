...
void irr::newton::IRagDoll::build(
	...
	//ATTACH THE BODY BY THE HEAD LIKE IN THE EXAMPLE: UN-USEFUL IN GENERAL CASE
	/*
	//head bone special
	...
	headBody = this->findBone(this->head_bone_id)->getBody()->getNewtonBody();	

	// get the pivot point
	matrix4 mat;
	NewtonBodyGetMatrix (headBody, &mat.M[0]);
	vector3df pivot (mat.getTranslation() + vector3df (0.0f, 30.0f * IrrToNewton, 0.0f));

	// attach the body by the head
//  uncoment this to hand the doll by the head
	int i = rand() % 4;
	if (i == 1) {
		NewtonConstraintCreateBall(nWorld, &pivot.X, headBody, NULL);	
	}
	*/  
