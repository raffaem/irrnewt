...
void irr::newton::IUtils::calculateCarTiresPosition(SCar car,irr::core::vector3df position_offset) {
	position_offset.X=-position_offset.X;
	position_offset.Z=-position_offset.Z;

	//positiong front right tire
	core::vector3df tire_pos=car.ChassisBody->getNode()->getPosition()-position_offset;
	car.TireNodeFR->setPosition(tire_pos);

	//positiong front left tire
	tire_pos.Z=tire_pos.Z+(position_offset.Z*2);
	car.TireNodeFL->setPosition(tire_pos);

	//positiong back left tire
	tire_pos.X=tire_pos.X+(position_offset.X*2);
	car.TireNodeBL->setPosition(tire_pos);

	//positiong back right tire
	tire_pos.Z=tire_pos.Z-(position_offset.Z*2);
	car.TireNodeBR->setPosition(tire_pos);
}
...
newton::IBody* irr::newton::IUtils::launchCube() {
	...
	cube->setMaterialTexture(
		0,
		driver->getTexture("media/bowling_pin_texture.jpg"));
...
scene::ISceneNode* irr::newton::IUtils::buildDefaultRagdoll(
	...
	float headSize;
	IAnimatedMeshSceneNode *head_Bone;
	IAnimatedMeshSceneNode *spine0_Bone;
	IAnimatedMeshSceneNode *spine1_Bone;
	IAnimatedMeshSceneNode *rUpperArm_Bone;
	IAnimatedMeshSceneNode *rLowerArm_Bone;
	IAnimatedMeshSceneNode *lUpperArm_Bone;
	IAnimatedMeshSceneNode *lLowerArm_Bone;

	IAnimatedMeshSceneNode *rUpperThigh_Bone;
	IAnimatedMeshSceneNode *rLowerThigh_Bone;
	IAnimatedMeshSceneNode *lUpperThigh_Bone;
	IAnimatedMeshSceneNode *lLowerThigh_Bone;
	...
	spine1_Bone = (IAnimatedMeshSceneNode*)new IrrBox(spine0_Bone, smgr, 666, spineSize.X, spineSize.Y, spineSize.Z); 
	if(texture != NULL)
	...
	head_Bone = (IAnimatedMeshSceneNode*)new IrrBox(spine1_Bone, smgr, 666, headSize1.X, headSize1.Y, headSize1.Z); 
	if(texture != NULL)
	...
	rUpperArm_Bone = (IAnimatedMeshSceneNode*)new IrrBox(spine1_Bone, smgr, 666, armSize.X, armSize.Y, armSize.Z); 
	if(texture != NULL)
	...
	// Right Lower arm
	rLowerArm_Bone = (IAnimatedMeshSceneNode*)new IrrBox(rUpperArm_Bone, smgr, 666, armSize.X, armSize.Y, armSize.Z); 
	if(texture != NULL)
	...
	// Left Upper leg
	lUpperArm_Bone = (IAnimatedMeshSceneNode*)new IrrBox(spine1_Bone, smgr, 666, armSize.X, armSize.Y, armSize.Z); 
	if(texture != NULL)
	...
	// Left Lower leg
	lLowerArm_Bone = (IAnimatedMeshSceneNode*)new IrrBox(lUpperArm_Bone, smgr, 666, armSize.X, armSize.Y, armSize.Z); 
	if(texture != NULL)
	...
	rUpperThigh_Bone = (IAnimatedMeshSceneNode*)new IrrBox(spine0_Bone, smgr, 666, thighSize.X, thighSize.Y, thighSize.Z); 
	if(texture != NULL)
	...
	rLowerThigh_Bone = (IAnimatedMeshSceneNode*)new IrrBox(rUpperThigh_Bone, smgr, 666, calfSize.X, calfSize.Y, calfSize.Z); 
	if(texture != NULL)
	...
	// Left Upper leg
	//vector3df thighSize (headSize * 2.0f, headSize * 0.75f, headSize * 0.75f);
	lUpperThigh_Bone = (IAnimatedMeshSceneNode*)new IrrBox(spine0_Bone, smgr, 666, thighSize.X, thighSize.Y, thighSize.Z); 
	if(texture != NULL)
	...
	// Left Lower leg
	//vector3df calfSize (headSize * 2.0f, headSize * 0.65f, headSize * 0.65f);
	lLowerThigh_Bone = (IAnimatedMeshSceneNode*)new IrrBox(lUpperThigh_Bone, smgr, 666, calfSize.X, calfSize.Y, calfSize.Z); 
    if(texture != NULL)
