...
NewtonCollision* CreateCollisionTree(
		...
		nCollision = NewtonCreateTreeCollision(world->getNewtonWorld(), NULL);
		NewtonTreeCollisionBeginBuild(nCollision);
...
NewtonCollision* CreateCollision(
	...
	else if(nodeType == scene::ESNT_OCT_TREE || nodeType == scene::ESNT_UNKNOWN)
	...
	case EBT_CONVEX_HULL_MODIFIER:
		...
		nCollision = createConvexHullCollision(world, node, mesh_for_convex,offset);
		NewtonCreateConvexHullModifier(world->getNewtonWorld(), nCollision);
		break;
...
void ICollision::setUserID(u32 ID) {
	NewtonConvexCollisionSetUserID(this->n_collision,ID);
}

u32 ICollision::getUserID() {
	return NewtonConvexCollisionGetUserID(this->n_collision);
}
